...
export class DailyRegisterService {
  constructor(@inject('IDailyRegisterRepository') private repository: IDailyRegisterRepository) {}
  ...
  async createDailyRegister(data: Partial<IDailyRegister>) {
    ...
    const exists = await this.repository.existsByUserIdAndDate(formatObejectId(data.userId).toString());
    ...
    let endWork = new Date(getDateInManaguaTimezone().setHours(19, 30, 0, 0));
  ...
  async markExit(userId: string): Promise<IDailyRegister> {
    ...
    const updated = await this.repository.markExit(
      formatObejectId(register._id).toString(),
      getDateInManaguaTimezone()
    );
  ...
  async getBySucursalAndDateRange(
    ...
    return this.repository.findBySucursalAndDateRange(sucursalId, startDate.toJSDate(), endDate.toJSDate());
  }

  async updateWorkingHours(sucursalId: string, startWork: Date, endWork: Date) {
    const result = await this.repository.updateDailyRegistersBySucursal(sucursalId, {
      startWork,
      endWork,
    });
