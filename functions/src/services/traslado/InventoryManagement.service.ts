...
import {
  IAddQuantity,
  ICreateInventarioSucursal,
  IHandleStockProductBranch,
  IInit,
  IManageHerramientaModel,
  ISubtractQuantity,
  ISubtractQuantityLoop,
  TipoMovimientoInventario,
} from '../../interface/IInventario';
...
export class InventoryManagementService implements IManageHerramientaModel {
  ...
  async init({
    ...
    userId,
    listProductId,
    searchWithProductId = false,
  }: IInit): Promise<IInventarioSucursal[]> {
    this._listInventarioSucursal = searchWithProductId
      ? await this.inventarioSucursalRepo.getListProductByProductIds(branchId, listProductId as string[])
      : await this.inventarioSucursalRepo.getListProductByInventarioSucursalIds(branchId, listInventarioSucursalId);
  ...
  async subtractQuantity({
    ...
    isNoSave = false,
    tipoMovimiento,
  }: ISubtractQuantity): Promise<void | IInventarioSucursal> {
    const inventarioSucursal = this._listInventarioSucursal.find(
      (inventarioSucursal) =>
        (inventarioSucursal._id as mongoose.Types.ObjectId).toString() === inventarioSucursalId.toString()
    );

    if (!inventarioSucursal) throw new Error('Producto no encontrado en la sucursal');

    if (quantity > inventarioSucursal.stock) throw new Error('Cantidad a sustraer es mayor a la disponible.');
  ...
  async addQuantity({
    ...
    if (!inventarioSucursal) {
      ...
      if (!inventarioSucursalReal) throw new Error('Producto no encontrado en la sucursal');
      ...
      if (costoInventario !== costoUnitario) {
        ...
        let costoUnitarioTotal = multiplicarDecimal128(
          costoUnitario,
          new mongoose.Types.Decimal128(quantity.toString())
        );
  ...
  async handleStockProductBranch({ model, quantity, tipoMovimiento }: IHandleStockProductBranch): Promise<void> {
    const inventarioSucursal = await this.inventarioSucursalRepo.findBySucursalIdAndProductId(
      model.sucursalId.toString(),
      model.productoId.toString()
    );
    const productoGrupo = (await this.inventarioSucursalRepo.getProductoGrupoByProductoId(
      model.productoId.toString()
    )) as IProductosGrupos;
    ...
    inventarioSucursal
      ? await this.addQuantity(dataAddQuantity)
      : await this.createInventarioSucursal(dataCretaeInventarioSucursal);
  }
